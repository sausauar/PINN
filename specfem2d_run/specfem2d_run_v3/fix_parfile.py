#!/usr/bin/env python3
"""
fix_parfile.py — полный Par_file для вашей версии SpecFEM2D
Запускать из папки specfem2d_run_v3/:
    python3 fix_parfile.py
"""

import os, shutil

PARFILE_TEMPLATE = """\
# ============================================================
#  SpecFEM2D  Par_file — PINN acoustic
# ============================================================

title                           = PINN_acoustic_{LABEL}

# ---------- simulation type ----------
SIMULATION_TYPE                 = 1
NOISE_TOMOGRAPHY                = 0
SAVE_FORWARD                    = .false.

# ---------- time ----------
NSTEP                           = 14000
DT                              = 5.0d-05

# ---------- time scheme ----------
time_stepping_scheme            = 1

# ---------- axisymmetric ----------
AXISYM                          = .false.

# ---------- model ----------
setup_with_binary_database      = 0
MODEL                           = default
SAVE_MODEL                      = default
TOMOGRAPHY_FILE                 = ./DATA/tomo_file.xyz

# ---------- mesh ----------
PARTITIONING_TYPE               = 3
ngnod                           = 9
initialfield                    = .false.
add_Biot_absorption             = .false.
read_external_mesh              = .false.

# ---------- internal mesher ----------
NPROC                           = 1
interfacesfile                  = interfaces.dat

xmin                            = 0.d0
xmax                            = 1500.d0
nx                              = 150

absorbbottom                    = .true.
absorbright                     = .true.
absorbtop                       = .false.
absorbleft                      = .true.

nbmodels                        = 2
1 1 2700.d0 3000.d0 0.d0 0 0 0 0 0 0 0 0 9999. 9999.
2 1 2700.d0 2000.d0 0.d0 0 0 0 0 0 0 0 0 9999. 9999.

nbregions                       = 5
1  150  1   15  1
1  150  37  50  1
1  55   16  36  1
56  96  16  36  2
97  150 16  36  1

# ---------- SLS attenuation ----------
N_SLS                           = 3
ATTENUATION_f0_REFERENCE        = 5.196152d0
READ_VELOCITIES_AT_f0           = .false.
USE_SOLVOPT                     = .false.

# ---------- attenuation (off) ----------
ATTENUATION_VISCOACOUSTIC       = .false.
ATTENUATION_VISCOELASTIC        = .false.
ATTENUATION_PORO_FLUID_PART     = .false.
Q0_poroelastic                  = 1
freq0_poroelastic               = 10
Q0                              = 1.
freq0                           = 20.0
UNDO_ATTENUATION_AND_OR_PML     = .false.
NT_DUMP_ATTENUATION             = 500

# ---------- electromagnetic (off) ----------
ATTENUATION_PERMITTIVITY        = .false.
ATTENUATION_CONDUCTIVITY        = .false.
f0_electromagnetic              = 1000

# ---------- sources ----------
NSOURCES                        = 1
force_normal_to_surface         = .false.
ACOUSTIC_FORCING                = .false.
noise_source_time_function_type = 4
write_moving_sources_database   = .false.

# ---------- Bielak conditions (off) ----------
add_Bielak_conditions_bottom    = .false.
add_Bielak_conditions_right     = .false.
add_Bielak_conditions_top       = .false.
add_Bielak_conditions_left      = .false.

# ---------- seismograms ----------
seismotype                      = 1
NTSTEP_BETWEEN_OUTPUT_SEISMOS   = 14000
NTSTEP_BETWEEN_OUTPUT_SAMPLE    = 1
save_ASCII_seismograms          = .true.
save_binary_seismograms_single  = .false.
save_binary_seismograms_double  = .false.
SU_FORMAT                       = .false.
USE_TRICK_FOR_BETTER_PRESSURE   = .false.
NTSTEP_BETWEEN_OUTPUT_INFO      = 2000
USER_T0                         = 0.0d0

# ---------- kernels ----------
SAVE_ASCII_KERNELS              = .true.
NTSTEP_BETWEEN_COMPUTE_KERNELS  = 1
APPROXIMATE_HESS_KL             = .true.
NO_BACKWARD_RECONSTRUCTION      = .false.

# ---------- wavefields ----------
output_wavefield_dumps          = .true.
NTSTEP_BETWEEN_OUTPUT_WAVE_DUMPS= 100
use_binary_for_wavefield_dumps  = .false.
imagetype_wavefield_dumps       = 1

# ---------- images (off) ----------
output_color_image              = .false.
output_postscript_snapshot      = .false.
NTSTEP_BETWEEN_OUTPUT_IMAGES    = 2000
cutsnaps                        = 1.
imagetype_JPEG                  = 2
factor_subsample_image          = 1.0d0
USE_CONSTANT_MAX_AMPLITUDE      = .false.
CONSTANT_MAX_AMPLITUDE_TO_USE   = 1.17d4
POWER_DISPLAY_COLOR             = 0.30d0
DRAW_SOURCES_AND_RECEIVERS      = .true.
DRAW_WATER_IN_BLUE              = .true.
USE_SNAPSHOT_NUMBER_IN_FILENAME = .false.

# ---------- postscript (off) ----------
imagetype_postscript            = 1
meshvect                        = .true.
modelvect                       = .false.
boundvect                       = .true.
interpol                        = .true.
pointsdisp                      = 6
subsamp_postscript              = 1
sizemax_arrows                  = 1.d0
US_LETTER                       = .false.

# ---------- absorbing boundaries ----------
STACEY_ABSORBING_CONDITIONS     = .false.
PML_BOUNDARY_CONDITIONS         = .true.
NELEM_PML_THICKNESS             = 10
ROTATE_PML_ACTIVATE             = .false.
rotate_PML_angle                = 0.
K_MIN_PML                       = 1.d0
K_MAX_PML                       = 1.d0
damping_change_factor_acoustic  = 0.5d0
damping_change_factor_elastic   = 1.d0
PML_PARAMETER_ADJUSTMENT        = .false.

# ---------- periodic conditions (off) ----------
ADD_PERIODIC_CONDITIONS         = .false.
PERIODIC_HORIZ_DIST             = 1500.

# ---------- receivers ----------
use_existing_STATIONS           = .true.
nreceiversets                   = 1
anglerec                        = 0.d0
rec_normal_to_surface           = .false.

# ---------- P-SV ----------
P_SV                            = .true.

# ---------- energy ----------
OUTPUT_ENERGY                   = .false.
NTSTEP_BETWEEN_OUTPUT_ENERGY    = 10
COMPUTE_INTEGRATED_ENERGY_FIELD = .false.

# ---------- simultaneous runs ----------
NUMBER_OF_SIMULTANEOUS_RUNS     = 1
BROADCAST_SAME_MESH_AND_MODEL   = .true.

# ---------- GPU (off) ----------
GPU_MODE                        = .false.

# ---------- misc ----------
TURN_ANISOTROPY_ON              = .false.
"""


def fix_parfile(src_dir, label):
    par_path = os.path.join(src_dir, "DATA", "Par_file")
    backup   = par_path + ".bak"

    if not os.path.isfile(par_path):
        print(f"  [SKIP] {par_path} not found")
        return

    shutil.copy2(par_path, backup)
    print(f"  Backup → {backup}")

    with open(par_path, "w") as f:
        f.write(PARFILE_TEMPLATE.replace("{LABEL}", label))

    print(f"  Fixed  → {par_path}")


def main():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    fixed = 0

    for candidate in [script_dir, os.path.join(script_dir, "specfem2d_run_v3")]:
        for src in ["src0", "src1"]:
            path = os.path.join(candidate, src)
            if os.path.isdir(path):
                print(f"\nFixing {path} ...")
                fix_parfile(path, src)
                fixed += 1

    if fixed == 0:
        print("ERROR: src0/src1 not found. Run from specfem2d_run_v3/")
    else:
        print(f"\nDone ({fixed} dirs patched). Now run:")
        print("  bash run_specfem.sh")


if __name__ == "__main__":
    main()